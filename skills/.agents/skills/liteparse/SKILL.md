...
```bash
# Basic text extraction (--num-workers 1 is REQUIRED to avoid OOM)
lit parse document.pdf --num-workers 1

# JSON output saved to a file
lit parse document.pdf --num-workers 1 --format json -o output.json

# Specific page range
lit parse document.pdf --num-workers 1 --target-pages "1-5,10,15-20"

# Disable OCR (faster, text-only PDFs)
lit parse document.pdf --no-ocr

# Use an external HTTP OCR server for higher accuracy
lit parse document.pdf --num-workers 1 --ocr-server-url http://localhost:8828/ocr

# Higher DPI for better quality
lit parse document.pdf --num-workers 1 --dpi 300
```
...
| `--num-workers <n>` | Pages to OCR in parallel (default: CPU cores - 1). **Always use `--num-workers 1`** to avoid OOM crashes on memory-constrained machines. |

> **IMPORTANT:** Always pass `--num-workers 1` to every `lit parse` and `lit batch-parse` call. Running at full concurrency spawns one Tesseract worker per page simultaneously, consuming all available RAM and killing the process.
